...
#
# Because the defaults are derived from %USERPROFILE% and from this file's
# location (the bundle always lives at <WEB_NN>\route-a-webgpu-windows), the
# same bundle checkout works on any machine/account without edits.
#
# Environment variable overrides (only needed for non-standard layouts):
#   CHROMIUM_SRC        Chromium checkout      (default: %USERPROFILE%\workspace\chromium\src)
...
#   DEPOT_TOOLS         depot_tools dir        (default: %USERPROFILE%\workspace\depot_tools)
...
$DefaultChromiumSrc  = Join-Path $env:USERPROFILE "workspace\chromium\src"
...
$DefaultDepotTools   = Join-Path $env:USERPROFILE "workspace\depot_tools"
