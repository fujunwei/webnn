...
// Vendored copy of Dawn's generated src/dawn/dawn_proc.cpp (see
// route-a-webgpu-windows/README.md, patch 01) — the upstream file pulls in
// Dawn's internal (non-public) headers src/dawn/common/Compiler.h,
// src/utils/assert.h and src/utils/log.h, none of which ship in the prebuilt
// webgpu_dawn include tree. Those three are only used for a sanitizer-disable
// attribute, a version-mismatch crash check, and a warning-log stream, so
// they're replaced below with trivial local equivalents; the actual proc
// table plumbing (dawnProcSetProcs / wgpu* trampolines) is untouched.
#if defined(__clang__)
#define DAWN_NO_SANITIZE(x) __attribute__((no_sanitize(x)))
#else
#define DAWN_NO_SANITIZE(x)
#endif

#define DAWN_CHECK(cond)                                                   \
    do {                                                                   \
        if (!(cond)) {                                                     \
            std::fprintf(stderr,                                           \
                          "dawn_proc: CHECK failed: %s (%s:%d)\n", #cond,   \
                          __FILE__, __LINE__);                             \
            std::abort();                                                  \
        }                                                                  \
    } while (0)
...
DawnProcTable MakeNullProcTable() {
    DawnProcTable procs = {};
    for (size_t i = 0; i < dawn::kDawnVersion.size(); ++i) {
        procs.version[i] = dawn::kDawnVersion[i];
    }
...
void dawnProcSetProcs(const DawnProcTable* procs_) {
    if (procs_) {
        ...
        for (size_t i = 0; i < dawn::kDawnVersion.size(); ++i) {
            if (procs_->version[i] != dawn::kDawnVersion[i]) {
                version_matches = false;
                break;
            }
        }
        DAWN_CHECK(version_matches);
        procs = *procs_;
    } else {
        procs = kNullProcs;
    }
